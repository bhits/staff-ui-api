...
+ To build a Docker Image (this will create an image with `bhitsdev/staff-ui-api:latest` tag):
...
+ Run as a Docker Container: `docker run -d bhitsdev/staff-ui-api:latest <additional program arguments>`
...
+ `docker run -d bhitsdev/staff-ui-api:latest --server.port=80`
...
...
  staff-ui-api.c2s.com:
    image: "bhitsdev/staff-ui-api:latest"
...
+ `docker run -d -v "/path/on/dockerhost/ssl_keystore.keystore:/path/to/ssl_keystore.keystore" bhitsdev/staff-ui-api:latest --spring.profiles.active=ssl --server.ssl.key-store=/path/to/ssl_keystore.keystore --server.ssl.key-store-password=strongkeystorepassword`
+ In a `docker-compose.yml`, this can be provided as:
```yml
version: '2'
services:
...
Please use [GitHub Issues](https://github.com/bhits-dev/staff-ui-api/issues) page to report issues.
